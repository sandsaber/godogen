...
Publishing writes `AGENTS.md` plus `.agents/skills/` with `agents/metadata.yaml` generated from each skill's `SKILL.md` frontmatter. Works with any coding agent.
...
**Agent-agnostic.** Published repos work with any coding agent — Claude Code, Codex, Cursor, OpenCode, Cline, and others.
