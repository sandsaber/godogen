...
The root [publish.sh](publish.sh) renders the right runtime layout for the chosen engine. Works with any coding agent — Claude Code, Codex, Cursor, OpenCode, Cline, Gemini CLI, Windsurf, Continue, and others. See [AI tool compatibility](docs/AI_TOOLS.md).

Published game repos use `AGENTS.md` as the canonical instruction file and `.agents/skills/` as the canonical runtime skill directory. Tool-specific files such as `CLAUDE.md`, `GEMINI.md`, `opencode.jsonc`, Cursor rules, Cline rules, Windsurf rules, and Continue rules are generated as thin bootstrap shims that point back to the same Godogen runtime.
...
- `--agent-compat full|minimal|none` — choose generated AI-tool bootstrap files; default is `full`
- `--dry-run` — show what would be written without changing the target

Compatibility modes:

- `full` — `GODOGEN.md`, `opencode.jsonc`, `CLAUDE.md`, `GEMINI.md`, Cursor, Cline, Windsurf, and Continue shims
- `minimal` — only `GODOGEN.md` and `opencode.jsonc`
- `none` — only `AGENTS.md` and `.agents/skills/`
...
Check publish behavior without touching the target:

```bash
./publish.sh --engine godot --merge --dry-run --out ~/existing-project
```

Run the publish smoke test after changing publish scripts:

```bash
scripts/test_publish_smoke.sh
```

### Published repo layout

After publishing, the target repo contains:

- `AGENTS.md` — canonical runtime instructions
- `GODOGEN.md` — plain markdown start-here file for tools without a skill protocol
- `.agents/skills/godogen/` — the main generation pipeline
- `.agents/skills/godot-api/` or `.agents/skills/bevy-help/` — engine-specific reference skill
- `.agents/hooks/` — capture and post-task helper hooks
- optional AI-tool bootstrap files, controlled by `--agent-compat`

Generated compatibility files contain a `Generated by godogen publish.sh.` marker. Re-publishing updates files with that marker and preserves user-owned files without it.
