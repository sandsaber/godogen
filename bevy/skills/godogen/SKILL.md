...
description: |
  Generate or update a complete Bevy game from a natural-language description. Use when the user wants to make, rebuild, or substantially extend a Bevy project end to end.
---
...
User request
    ...
    |   +- Read capture.md, produce a fresh screenshots/result/{N}/ bundle
    |
...
**If a budget is provided, generating proper assets is part of the task, not optional.** Do not fall back to procedural primitives when the budget allows a real asset. Plan and generate through `asset-planner.md` / `asset-gen.md`.

Procedural stand-ins are acceptable only for genuinely abstract shapes or when explicitly ruled out on budget grounds.
...
If `PLAN.md` calls for presentation media, finish through the Bevy capture flow and leave a fresh `screenshots/result/{N}/` proof bundle behind.

## Bevy Help

When you need Bevy-specific help, use the `bevy-help` skill with a targeted query. It keeps large reference material out of the main pipeline while giving you version-aware access to rustdoc, examples, and docs.

Be specific about what you need:

- **Targeted query** — ask for a concrete symbol or pattern: `"Camera3d: what is the Bevy 0.18 pattern?"`
